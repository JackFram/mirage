...
def reset_tensors(dist_param: ProcessGroupInfo):
    ...
    num_tokens_per_rank, hidden_dim, inter_dim, num_topk, num_experts = 64, 2560, 1280, 8, 32
    ...
    ffn_fused_w13_output_tensor = torch.randn(
        (num_local_experts, num_tokens, inter_dim),
        dtype=torch_dtype(moe_param.in_dtype),
    ).cuda().permute(permute_order)
    ...
    """
    Reference implementation for ffn-w2-send and combine task here:
    """
    topk_indices_ref = topk_indices.clone()
    w2_tensor_ref = w2_tensor.clone()
    ffn_fused_w13_output_tensor_ref = ffn_fused_w13_output_tensor.clone()
    topk_weights_ref = topk_weights.clone()
    output_tensor_ref = output_tensor.clone() # ffn-w2-combine ut output
    ...
    ffn_w2_output_ref = torch.einsum("mkl,nkl->mnl", ffn_fused_w13_output_tensor_ref, w2_tensor_ref)
    ffn_w2_output_ref = ffn_w2_output_ref.permute(2, 0, 1)
    ffn_w2_output_ref_list = [torch.empty_like(ffn_w2_output_ref) for _ in range(num_ranks)]
    dist.all_gather(ffn_w2_output_ref_list, ffn_w2_output_ref)
    gathered_ffn_w2_output_ref = torch.cat(ffn_w2_output_ref_list, dim=0)

    # prepare combine info tensor for ffn_w2_send and get reference output
    global_token_count = torch.zeros((num_ranks, num_local_experts), dtype=torch.int32)
    for token_idx in range(gathered_topk_indices_ref.shape[0]):
        for topk_idx in range(num_topk):
            expert_idx = gathered_topk_indices_ref[token_idx, topk_idx].item()
            src_rank_idx = token_idx // num_tokens_per_rank
    ...
    for expert_idx in range(num_local_experts):
        for token_tile_idx in range(num_tokens // moe_param.mma_tiler_mn[1]):
            for ffn_task_id in range(hidden_dim // moe_param.mma_tiler_mn[0]):
                mpk_task_queue[task_idx][0] = (MPKTask.kFusedFFNW2Send.value << 28) | (expert_idx << 16) | (token_tile_idx << 8) | ffn_task_id
                task_idx += 1
    for token_idx in range(num_tokens_per_rank):
        mpk_task_queue[task_idx][0] = (MPKTask.kCombineRecv.value << 28) | token_idx
        task_idx += 1
